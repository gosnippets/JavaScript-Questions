...
- A: `Maya` and `undefined`
- B: `Maya` and `ReferenceError`
...
The string `'Maya'` is a truthy value. What we're actually asking, is "is this truthy value falsy?". This returns `false`.
...
const member = new Person('Maya', 'Devi');
...
const Maya = new Person('Maya', 'Devi');
...
- A: `Person {firstName: "Maya", lastName: "Devi"}` and `undefined`
- B: `Person {firstName: "Maya", lastName: "Devi"}` and `Person {firstName: "Sarah", lastName: "Smith"}`
- C: `Person {firstName: "Maya", lastName: "Devi"}` and `{}`
- D: `Person {firstName: "Maya", lastName: "Devi"}` and `ReferenceError`
...
- A: `"Maya"` `21` `["", " is ", " years old"]`
- B: `["", " is ", " years old"]` `"Maya"` `21`
- C: `"Maya"` `["", " is ", " years old"]` `21`
...
```javascript
String.prototype.giveMayaPizza = () => {
  return 'Just give Maya pizza already!';
};
...
- A: `"Just give Maya pizza already!"`
...
- A: `undefined is 21` `Maya is 21`
- B: `function` `function`
- C: `Maya is 21` `Maya is 21`
- D: `Maya is 21` `function`
...
The value of `person` is an object. The argument `member` has a (copied) reference to the _same_ object. When we modify a property of the object `member` has a reference to, the value of `person` will also be modified, since they both have a reference to the same object. `person`'s `name` property is now equal to the value `"Maya"`
...
- A: `{ admin: true, user: { name: "Maya", age: 21 } }`
- B: `{ admin: true, name: "Maya", age: 21 }`
- C: `{ admin: true, user: ["Maya", 21] }`
- D: `{ admin: true }`
...
It's possible to combine objects using the spread operator `...`. It lets you create copies of the key/value pairs of one object, and add them to another object. In this case, we create copies of the `user` object, and add them to the `admin` object. The `admin` object now contains the copied key/value pairs, which results in `{ admin: true, name: "Maya", age: 21 }`.
...
- A: `{ name: "Maya", age: 21 }`, `["name", "age"]`
- B: `{ name: "Maya", age: 21 }`, `["name"]`
- C: `{ name: "Maya"}`, `["name", "age"]`
- D: `{ name: "Maya"}`, `["age"]`
...
- B: `"{"username": "MayaDevi"}"`
- C: `"["level", "health"]"`
- D: `"{"username": "MayaDevi", "level":19, "health":90}"`
...
```javascript
const name = 'Maya Devi';
console.log(name.padStart(13));
console.log(name.padStart(2));
```

- A: `"Maya Devi"`, `"Maya Devi"`
- B: `" Maya Devi"`, `" Maya Devi"` (`"[13x whitespace]Maya Devi"`, `"[2x whitespace]Maya Devi"`)
- C: `" Maya Devi"`, `"Maya Devi"` (`"[1x whitespace]Maya Devi"`, `"Maya Devi"`)
- D: `"Maya Devi"`, `"Lyd"`,
...
With the `padStart` method, we can add padding to the beginning of a string. The value passed to this method is the _total_ length of the string together with the padding. The string `"Maya Devi"` has a length of `12`. `name.padStart(13)` inserts 1 space at the start of the string, because 12 + 1 is 13.
...
When we unpack the property `name` from the object on the right-hand side, we assign its value `"Maya"` to a variable with the name `myName`.

With `{ name: myName }`, we tell JavaScript that we want to create a new variable called `myName` with the value of the `name` property on the right-hand side.

Since we try to log `name`, a variable that is not defined, `undefined` is returned on the left side assignment. Later, the value of `Maya` is stored through the destructuring  assignment. 
...
`function sayHi(name = "Maya") { ... }`

In this case, if we didn't pass a value or if we passed `undefined`, `name` would always be equal to the string `Maya`
...
- A: `{ name: "Maya", age: 21 }`
- B: `{ name: "Maya", age: 21, city: "Amsterdam" }`
- C: `{ name: "Maya", age: 21, city: undefined }`
- D: `"Amsterdam"`
...
- A: `{ default: function default(), name: "Maya" }`
- B: `{ default: function default() }`
- C: `{ default: "Hello world", name: "Maya" }`
...
```javascript
function giveMayaPizza() {
  return 'Here is pizza!';
}

const giveMayaChocolate = () =>
  "Here's chocolate... now go hit the gym already.";

console.log(giveMayaPizza.prototype);
console.log(giveMayaChocolate.prototype);
```
...
Regular functions, such as the `giveMayaPizza` function, have a `prototype` property, which is an object (prototype object) with a `constructor` property. Arrow functions however, such as the `giveMayaChocolate` function, do not have this `prototype` property. `undefined` gets returned when trying to access the `prototype` property using `giveMayaChocolate.prototype`.
...
- A: `name` `Maya` and `age` `21`
- B: `["name", "Maya"]` and `["age", 21]`
...
The first subarray is `[ "name", "Maya" ]`, with `x` equal to `"name"`, and `y` equal to `"Maya"`, which get logged.
...
- B: `[1, [2, 3, 4]]` and `{ name: "Maya", age: 21 }`
- C: `[1, 2, 3, 4]` and `{ name: "Maya", age: 21 }`
- D: `Error` and `{ name: "Maya", age: 21 }`
...
- A: `3`, `NaN`, `NaN`
- B: `3`, `7`, `NaN`
- C: `3`, `Maya2`, `[object Object]2`
- D: `"12"`, `Maya2`, `[object Object]2`
...
However, the second one is a string `"Maya"`. `"Maya"` is a string and `2` is a number: `2` gets coerced into a string. `"Maya"` and `"2"` get concatenated, which results in the string `"Maya2"`.

`{ name: "Maya" }` is an object. Neither a number nor an object is a string, so it stringifies both. Whenever we stringify a regular object, it becomes `"[object Object]"`. `"[object Object]"` concatenated with `"2"` becomes `"[object Object]2"`.
...
const jsonArray = JSON.stringify({ name: 'Maya' }); // '{"name":"Maya"}'
JSON.parse(jsonArray); // { name: 'Maya' }
```
...
If we wouldn't have declared the `name` variable within the `getName` function, the javascript engine would've looked down the _scope chain_. The outer scope has a variable called `name` with the value of `Maya`. In that case, it would've logged `Maya`.
...
First, we invoke the `changeAge` function and pass the `person` object as its argument. This function increases the value of the `age` property by 1. `person` is now `{ name: "Maya", age: 22 }`.

Then, we invoke the `changeAgeAndName` function, however we don't pass a parameter. Instead, the value of `x` is equal to a _new_ object: `{ ...person }`. Since it's a new object, it doesn't affect the values of the properties on the `person` object. `person` is still equal to `{ name: "Maya", age: 22 }`.
...
- B: `Mara` `undefined` `Maya Devi` `ReferenceError`
- C: `Mara` `null` `Maya Devi` `null`
...
`person.getFullName?.()`: `person` has a property named `getFullName`: `person.getFullName()` is not nullish and can get invoked, which returns `Maya Devi`.
...
First, we invoke `funcOne`. On the first line of `funcOne`, we call the _asynchronous_ `setTimeout` function, from which the callback is sent to the Web API. (see my article on the event loop <a href="https://dev.to/MayaDevi/javascript-visualized-event-loop-3dif">here</a>.)
...
First, we added a new property `name` to the proxy object (`person.name = "Maya"`). `set` gets invoked, and logs `"Added a new property!"`.
...
###### 144. What do we need to add to the `person` object to get `["Maya Devi", 21]` as the output of `[...person]`?
...
Objects aren't iterable by default. An iterable is an iterable if the iterator protocol is present. We can add this manually by adding the iterator symbol `[Symbol.iterator]`, which has to return a generator object, for example by making it a generator function `*[Symbol.iterator]() {}`. This generator function has to yield the `Object.values` of the `person` object if we want it to return the array `["Maya Devi", 21]`: `yield* Object.values(this)`.
...
###### 153. What should the value of `method` be to log `{ name: "Maya", age: 22 }`? 

```javascript
const keys = ["name", "age"]
const values = ["Maya", 22]
...
This creates an array of subarrays containing the correct keys and values, which results in `{ name: "Maya", age: 22 }`
